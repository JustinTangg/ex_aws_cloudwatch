if Code.ensure_loaded?(SweetXml) do
  defmodule ExAws.Cloudwatch.Parsers do
    ...
    def parse({:ok, %{body: xml}= resp}, :describe_alarm_history) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//DescribeAlarmHistoryResponse",
    ...
    def parse({:ok, %{body: xml}= resp}, :get_dashboard) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//GetDashboardResponse",
    ...
    def parse({:ok, %{body: xml}= resp}, :get_metric_statistics) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//GetMetricStatisticsResponse",
    ...
    def parse({:ok, %{body: xml}= resp}, :list_dashboards) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//ListDashboardsResponse",
    ...
    def parse({:ok, %{body: xml}= resp}, :list_metrics) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//ListMetricsResponse",
    ...
    def parse({:ok, %{body: xml}= resp}, :put_dashboard) do
      parsed_body = xml
                    |> SweetXml.xpath(~x"//PutDashboardResponse",
    ...
    [
      ...
      extended_statistics: ~x"./ExtendedStatistics/text()"m, #TODO probably fix this
